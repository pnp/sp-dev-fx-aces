...
> {
  ...
  public onInit(): Promise<void> {
    let podcastsContent: PnPPodcasts;
    const podcastsService: PnPPodcastsService = new PnPPodcastsService();
    return podcastsService.getRSSContent(podcastsContent, this.context.httpClient).then((podcastsContent: PnPPodcasts) => {
      this.state = {
        ID: 1,
        SearchText: "",
        Items: podcastsContent.channel.item,
        channel: podcastsContent.channel,
      };
